...
tic
[A_ref0,Ap_ref,Am_ref,p,n,k,N,x,dx,lam_start]=spatial_pencil_cvgce_initial_res; %loads example
...
M=200; % highest order of eigenspace to be computed in initial approximation, determines polynomial approximation order of operator pencil
...
M_fine=5; % not so high order iteration when already near singularity

tol=-1e-18; % tolerance in initial approximation; accuracy of PGM; aim for 1e-6 if not sure, should use M iterations
...
max_fine=200; % maximum number of iterations in fine loop with order of penciil truncated to M_fine
max_newton = 20; % max iterations in Newton; sert to zero if want to avoid Newton


verbose=1; 

s_f_init=.7; % initial step should not be too large when first exploring, problem when large dlam with converging power series, can be close to 1 later but risk hitting abs spec with ambiguity of eigenspaces; larger M allows for larger s_f_init            
s_f_fine=0.8; % step fraction in power method, avoid overshoot so chosen close to but below 1
...
for j=p+2:M+1
     iota(1:n*(N+2),(1:n*(N+2))+(j-1)*n*(N+2))=   [sparse(n*N,n*(N+2));...
        sparse(n,n), sparse(n,n*N), sparse(n,n-k),sparse(Eu(1:n,(1:k)+(j-1)*(k)));...
        sparse(n,n*N),sparse(n,n), sparse(Es(1:n,(1:n-k)+(j-1)*(n-k))), sparse(n,k)];
end
...
plot(real(log10(lam_list+lam_start+2)),'.-')
...
name=['data_plotting/ac_lam_start_cvgce_initial_res_' num2str(lam_start) '_M_' num2str(M) '.mat'];
...
while (abs(dlam_o)+abs(dlam)>tol_fine) && (it<max_fine)
    ...
    [UEs,~]=qr(Es(1:n,1:n-k));
%      A0=A(:,:,1);
    UEu=eu_newton(Am(1:n,1:n),UEu,n,k,tol_subspace);
    UEs=eu_newton(Ap(1:n,1:n),UEs,n,n-k,tol_subspace);
...
%          break
%      end
    [dlam,u_eig,err,M_final]=pencil_eig(iota,u0,n*(N+2),tol_fine,M_fine);
