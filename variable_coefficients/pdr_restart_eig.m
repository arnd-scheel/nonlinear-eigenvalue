...
M=40 ; % highest order of eigenspace to be computed in initial approximation, determines polynomial approximation order of operator pencil
...
M_fine=3; % not so high order iteration when already near singularity but sufficiently high when finding branch point
...
for j=1:p+1
    iota(1:n*(N+2),(1:n*(N+2))+(j-1)*n*(N+2))=   [A(1:n*N,(1:n*(N+1))+(j-1)*n*(N+1)), sparse(n*N,n);...
                    (j==1)*speye(n,n), sparse(n,n*N), sparse(Es(1:n,(1:n-k)+(j-1)*(n-k))), sparse(n,k);...
                    sparse(n,n*N),(j==1)*speye(n,n), sparse(n,n-k),sparse(Eu(1:n,(1:k)+(j-1)*(k)))];
end
for j=p+2:M+1
     iota(1:n*(N+2),(1:n*(N+2))+(j-1)*n*(N+2))=   [sparse(n*N,n*(N+2));...
                    sparse(n,n), sparse(n,n*N), sparse(Es(1:n,(1:n-k)+(j-1)*(n-k))), sparse(n,k);...
                    sparse(n,n*N),sparse(n,n), sparse(n,n-k),sparse(Eu(1:n,(1:k)+(j-1)*(k)))];
end
...
plot(real(log10(lam_list)),'.-')
%  plot(real((lam_list)),'.-')

%  plot(log10(1:length(lam_list))',real(log10(lam_list+lam_start+2)),'.-')

name=['ac_lam_start_restart_' num2str(lam_start) '_M_' num2str(M) '.mat'];
