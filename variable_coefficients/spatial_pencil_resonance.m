...
% A(x; *), Ap(*), and Am(*) are polynomial of degree p in lam, with expansion as in 
...
% resonances with potential F0 sech^2 x are at gam=-1/2 pm sqrt(F0+1/4); Evans function is analytic up until gam=-1
F0=-0.1;
a=F0*sech(x).^2; 

display(['predicted resonance is at ' num2str(-1/2 +sqrt(F0+1/4))])
%%% if want to eliminate root at branch point 
...
Ap=sparse(n,n*(p+1));
Ap(1:n,1:n)=sparse([0 1; 0 0]);
Ap(2,1+(3-1)*2)=1; % how does the (linear) parameter enter
...
for j=1:N+1
    ...
    A(2*j-1:2*j,(2*j-1:2*j) +(3-1)*n*(N+1))=sparse([0 0; 1 0]); 
end
